...
export default function ProductReports() {
    ...
    return (
        <div className="p-6 bg-white rounded shadow mt-8">
            <h2 className="text-xl mb-4">Product Reports</h2>
            <table className="w-full text-left border-collapse">
                ...
                {reports.map(r => (
                    <tr key={r.id}>
                        <td className="border p-2">{r.productId}</td>
                        <td className="border p-2">{r.reporterUserId}</td>
                        <td className="border p-2">{r.reason}</td>
                        <td className="border p-2 space-x-2">
                            <button onClick={() => banUser(r.reporterUserId)} className="px-2 py-1 bg-red-500 text-white rounded">Ban</button>
                            <button onClick={() => sendWarning(r.reporterUserId, r.reason)} className="px-2 py-1 bg-yellow-400 rounded">Warn</button>
                            <button onClick={() => deleteProduct(r.productId)} className="px-2 py-1 bg-gray-700 text-white rounded">Delete</button>
                        </td>
                    </tr>
                ))}
                </tbody>
